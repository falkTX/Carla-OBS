...
static int carla_audio_gen_thread(void *data)
{
	...
	while (priv->audiogen_running) {
		const uint64_t slice =
			priv->bufferSize * 1000000000ULL / priv->sampleRate;
		...
		if (priv->activated) {
			...
			priv->descriptor->process(priv->handle, silences, bufs,
						  out.frames, NULL, 0);
		...
		const struct timespec timeout = {target / 1000000000ULL,
				     ...
				     uint32_t srate, bool filter)
{
	...
	priv->internalHostHandle = carla_create_native_plugin_host_handle(
		descriptor, priv->handle);
	if (priv->internalHostHandle == NULL)
		goto fail2;

	descriptor->dispatcher(priv->handle,
			       NATIVE_PLUGIN_OPCODE_HOST_USES_EMBED, 0, 0, NULL,
			       0.f);

	// TODO build and setup local bridges
	carla_set_engine_option(priv->internalHostHandle,
	...
	if (!filter) {
		...
		thrd_create(&priv->audiogen_thread, carla_audio_gen_thread,
			    priv);
	}
				 ...
				 obs_properties_t *props, bool reset)
{
	...
	if (carla_get_current_plugin_count(priv->internalHostHandle) != 0 &&
	    carla_get_plugin_info(priv->internalHostHandle, 0)->hints &
				       ...
				       obs_property_t *property, void *data)
{
	...
	if (carla_add_plugin(priv->internalHostHandle, plugin->build,
